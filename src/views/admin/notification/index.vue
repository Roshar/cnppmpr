                    ...
                    <div class="widget-49">
                        <table class="table">
                            ...
                            <tr>
                                ...
                                <th scope="col">Отменить запрос</th>
                                <th scope="col">Удалить</th>
                            </tr>
                            </thead>
                            <tbody>

                                <tr  v-if="requestIom.length" v-for="(item, index) in requestIom">
                                    ...
                                    <td>
                                        <ul class="list-top-menu-li inline">
                                            <li>
                                                <button class="btn btn-success" title="Отменить" @click="cancel(item.iom_id, item['tutor_id'])"> <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-card-text" viewBox="0 0 16 16">
                                ...
                                <tr v-else>
                                    <th scope="row" rowspan="5" style="text-align:center"> Активных запросов не обнаружено</th>
                                </tr>
...
<script>
    ...
    export default {
        setup() {
            ...
            const cancel = async (idIom,idTutor) => {

                await store.dispatch('notification/cancelRequest',{idIom, idTutor})
                requestIom.value = await store.dispatch('notification/getIomRequest')
                await router.push('/admin_notifications')
            }
